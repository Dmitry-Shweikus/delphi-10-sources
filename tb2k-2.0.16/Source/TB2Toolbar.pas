  ...
  if (Code = hpGetMessage) and (WParam = PM_REMOVE) then begin
    ...
    if (Msg.message = WM_SYSCHAR) and (Msg.hwnd <> 0) then begin
      ...
      if Assigned(MainForm) and MainForm.HandleAllocated and (GetCapture = 0) and
        ...
        SendMessage(MainForm.Handle, WM_SYSCOMMAND, SC_KEYMENU, Msg.WParam);
        Msg.message := WM_NULL;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    if Key = #0 then begin
      ...
      if I = nil then Exit;
      FView.Selected := I;
      FView.EnterToolbarLoop([tbetKeyboardControl]);
    end
    else begin
      ...
      if not IsOnlyItemWithAccel or
         (tbisSubmenu in TTBCustomItemAccess(Item).ItemStyle) then begin
        if IsOnlyItemWithAccel then begin
          FView.Selected := I;
          FView.EnterToolbarLoop([tbetDropDownMenus, tbetKeyboardControl]);
        end
        else
          if FMenuBar then begin
            FView.Selected := I;
            FView.EnterToolbarLoop([tbetKeyboardControl]);
          end;
      end
      else
        TTBCustomItemAccess(Item).Click;
    end;
    Result := True;
  finally
    if Assigned(I) then
