...

{
  ...
  $jrsoftware: tb2k/Source/TB2DsgnItemEditor.pas,v 1.48 2002/11/15 00:15:07 jr Exp $
}
...
const
  AboutText =
    '%s'#13#10 +
    'Copyright (C) 1998-2002 by Jordan Russell'#13#10 +
...
procedure TTBItemEditForm.ItemNotification(Sender: TTBCustomItem;
...
begin
  ...
  case Action of
    tbicInserted:
      begin
        if (Sender = FSelParentItem) and not Relayed then
          AddListViewItem(Index, Item);
        if IsSubmenuItem(Item) then
          RebuildTree;
      end;
    tbicDeleting:
      if (Sender = FSelParentItem) and not Relayed then begin
