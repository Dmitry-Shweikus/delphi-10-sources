...
var
  DT : Integer;
begin
  Result := _lopen2(lpPathname, OF_READ or OF_SHARE_DENY_NONE);
  if (Result = -1) then
    raise EAbFCIFileOpenError.Create;
...
function FDI_FileOpen(lpPathName: PChar; Flag, Mode: Integer) : Integer;
  ...
  {open a file}
begin
  Result := _lopen2(lpPathName, Mode);
end;
{ -------------------------------------------------------------------------- }
...
begin
  ...
  FH := _lopen2(FP, OF_READ or OF_SHARE_DENY_NONE);                       {!!.02}
