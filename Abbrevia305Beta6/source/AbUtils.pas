...
function _lcreat2(Path : PChar; Attr : Integer) : Integer;
function _lopen2(Path : PChar; iReadWrite : Integer) : Integer;
...
begin
  ws := Path;
  s := ws;
  Result := _lcreat(PAnsiChar(s),Attr);
end;
function _lopen2(Path : PChar; iReadWrite : Integer) : Integer;
var s : AnsiString;
    ws : String;
begin
  ws := Path;
  s := ws;
  Result := _lopen(PAnsiChar(s),iReadWrite);
end;
...
{$IFDEF MSWINDOWS}
var
  FileNameZ : array [0..259] of Ansichar;
  TempPathZ : array [0..259] of Ansichar;
{$ENDIF}
begin
{$IFDEF MSWINDOWS}
  if not AbDirectoryExists(Dir) then
    GetTempPathA(sizeof(TempPathZ), TempPathZ)
  else
    StrPCopy(TempPathZ, Dir);
  GetTempFileNameA(TempPathZ, 'VMS', Word(not CreateIt), FileNameZ);
  ...
  case AbGetPathType(ArchiveName) of
    ...
    ptAbsolute : begin {path is absolute}
      if Pos('\\', ArchiveName) = 1 then begin  {path is UNC; must refer to network }
        ...
        DrvTyp := GetDriveTypeW(PChar(DrvStr));                       {!!.02}
...
begin
  ...
  if (Pos('*', Path) <> 0) or (Pos('?', Path) <> 0) then
    Exit;
{$IFDEF MSWINDOWS}
  Attr := GetFileAttributesA( StrPCopy( PathZ, Path ) );
