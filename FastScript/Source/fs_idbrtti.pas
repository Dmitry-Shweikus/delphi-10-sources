...
begin
  ...
  begin
    if MethodName = 'LOADFROMFILE' then
      ...
      TBlobField(Instance).SaveToStream(TStream(Integer(Params[0])))
  end
  ...
  begin
    ...
    if MethodName = 'OPEN' then
      ...
      Result := Integer(_TDataSet.FieldByName(Params[0]))
    else if MethodName = 'GETFIELDNAMES' then
      _TDataSet.GetFieldNames(TStrings(Integer(Params[0])))
      ...
      _TDataSet.FreeBookmark(TBookMark(Integer(Params[0])))
    else if MethodName = 'GETBOOKMARK' then
      Result := Integer(_TDataSet.GetBookmark)
    else if MethodName = 'GOTOBOOKMARK' then
      _TDataSet.GotoBookmark(TBookMark(Integer(Params[0])))
    else if MethodName = 'LOCATE' then
      Result := _TDataSet.Locate(Params[0], Params[1], IntToLocateOptions(Params[2]))
  ...
  begin
    if MethodName = 'PARAMBYNAME' then
      Result := Integer(TParams(Instance).ParamByName(Params[0]))
    else if MethodName = 'FINDPARAM' then
      Result := Integer(TParams(Instance).FindParam(Params[0]))
    else if MethodName = 'ITEMS.GET' then
      Result := Integer(TParams(Instance)[Params[0]])
  end
end;
